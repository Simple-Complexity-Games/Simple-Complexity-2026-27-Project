...
var clear_texture = preload("res://Areas/Top-Down/LushWarden_Fog_Clearing_Mask.png")

var blit_material = BlitMaterial.new()
var fog_texture : DrawableTexture2D
var texture_size = Vector2i(1920, 1080)
var clearing_rect_size = Vector2i(300, 300)
...
func _process(delta: float) -> void:
	var player_position = Vector2i((Player.position.x / 4) - (clearing_rect_size.x / 2), 
	(Player.position.y / 4) - (clearing_rect_size.y / 2))
	
	fog_texture.blit_rect(Rect2i(player_position, clearing_rect_size), clear_texture, Color(1, 1, 1, 0.1), 0, blit_material)
	self.texture = fog_texture
